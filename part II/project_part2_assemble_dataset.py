...
def generate_dataset_combined_repair_hint(output_file_name):
    with open('./project_part2_dataset_training_raw.json', 'r') as raw_file:
        raw_data = json.load(raw_file)

    # attempt to combine all data and write it in json at the end
    combined_data_generated = list()
    ...
    for problem_id, examples in raw_data.items():
        for example in examples:
            ...
            # repair
            system_prompt_formatted = prompts.system_message_nus
            user_prompt_formatted = prompts.user_message_nus_repair_basic.format(problem_data=problem_data,
                                                                                 buggy_program=buggy_code)
            input_repair = f"""<|system|>\n{system_prompt_formatted}<|end|>\n<|user|>\n{user_prompt_formatted}<|end|>\n<|assistant|>"""
            output_repair = f"[FIXED]\n{repaired_code}\n[/FIXED]<|endoftext|>"
            combined_data_generated.append({"input": input_repair, "output": output_repair})

            # hint
            system_prompt_formatted = prompts.system_message_nus
            user_prompt_formatted = prompts.user_message_nus_hint_basic.format(problem_data=problem_data,
                                                                               buggy_program=buggy_code)
            input_hint = f"""<|system|>\n{system_prompt_formatted}<|end|>\n<|user|>\n{user_prompt_formatted}<|end|>\n<|assistant|>"""
            output_hint = f"[HINT]\n{hint}\n[/HINT]<|endoftext|>"
            combined_data_generated.append({"input": input_hint, "output": output_hint})

    random.shuffle(combined_data_generated)

    with open(output_file_name, 'w') as file:
        for combined_data_item in combined_data_generated:
            json_line = json.dumps(combined_data_item)
            file.write(json_line + '\n')

if __name__ == "__main__":    
    # mode = "repair"
    # generate_train_dataset(f'./project_part2_dataset_training_{mode}.jsonl', mode)
    generate_dataset_combined_repair_hint(f'./project_part2_dataset_training_combined_I15_I16.jsonl')
