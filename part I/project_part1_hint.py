...
class Hint:
    ...
    def __init__(self, model_name, is_huggingface):
        ...
        self.user_prompt_using_repair = prompts.user_message_nus_hint_using_repair
    ...
    def extract_explanation(self, text):
    ...
    def generate_hint(self, problem_data, buggy_program, testcases):
        ...
        try:
            ...
            # print(f"is_huggingFace = {self.is_huggingface}")
            repair_agent = repair.Repair(model_name=self.model_name, is_huggingface=self.is_huggingface)
            generated_repair = repair_agent.generate_repair(problem_data, buggy_program, testcases)

            if not generated_repair or generated_repair == "":
                print("Using the prompt when generated_repair is None or empty string.")
                formatted_user_prompt = self.user_prompt.format(
                    problem_data=problem_data,
                    buggy_program=buggy_program
                )
            else:
                print("Using the prompt when generated_repair is neither None nor empty string.")
                formatted_user_prompt = self.user_prompt_using_repair.format(
                    problem_data=problem_data,
                    buggy_program=buggy_program,
                    repaired_program=generated_repair
                )

            generated_response = self.call_llm(self.system_prompt, formatted_user_prompt)
            # print(generated_response)
            hint = self.extract_hint(generated_response)
            explanation = self.extract_explanation(generated_response)
            # print(f"hint = {hint}")
            # print(f"explanation = {explanation}")
        except Exception as ex:
            print(f"Exception occurred while generating and/or explanation. Exception = {ex}")
